...
def creardispositivos( numeroDispositivos, lambdaRegular_Tipo,tipo):
    dispositivos = []
    for disp in range(0, numeroDispositivos):  # Generamos la cantidad indicada de dispositivos de tipo 1
        ...
        dispositivos.append(
            DeviceMTC(lambdaRegular_Tipo, np.random.uniform(0, 100, 1), np.random.uniform(0, 100, 1), 0,tipo))
